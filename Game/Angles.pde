...
void computeAngles() {
  ...
  for (int[] quad : quads) {
    ...
    if (area < max_area && area > min_area && quadGraph.isConvex(c12, c23, c34, c41)) {
      ...
      corners[0] = c12; corners[1] = c23; corners[2] = c34; corners[3] = c41;
    }
  }
  ...
  for (int j = 0; j < corners.length; j++) output.add(corners[j]);

  angles = new TwoDThreeD(detect.width, detect.height).get3DRotations(quadGraph.sortCorners(output));
}